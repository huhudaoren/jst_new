...
<template>
  <view class="el-page">
    <scroll-view class="el-tabs" scroll-x>
      <view
        v-for="tab in tabs"
        :key="tab.value"
...
.el-page { min-height: 100vh; background: var(--jst-color-page-bg); }
.el-tabs { white-space: nowrap; background: var(--jst-color-card-bg); border-bottom: 2rpx solid var(--jst-color-border); }
.el-tabs__item { display: inline-block; padding: 0 32rpx; height: 88rpx; line-height: 88rpx; font-size: 26rpx; color: var(--jst-color-text-tertiary); position: relative; }
.el-tabs__item--active { color: #F5A623; font-weight: 700; }
...
.el-card { margin: 20rpx 32rpx 0; padding: 28rpx 32rpx; background: var(--jst-color-card-bg); border-radius: var(--jst-radius-md); box-shadow: var(--jst-shadow-card); }
.el-card__head { display: flex; justify-content: space-between; align-items: center; margin-bottom: 16rpx; }
.el-card__no { font-size: 24rpx; color: var(--jst-color-text-tertiary); }
.el-card__status { padding: 4rpx 16rpx; border-radius: var(--jst-radius-full); font-size: 22rpx; background: var(--jst-color-warning-soft); color: var(--jst-color-warning); }
.el-card__status--paid, .el-card__status--pending_ship, .el-card__status--shipped { background: var(--jst-color-brand-soft); color: var(--jst-color-brand); }
.el-card__status--completed { background: var(--jst-color-success-soft); color: var(--jst-color-success); }
.el-card__status--cancelled { background: var(--jst-color-gray-soft); color: var(--jst-color-text-tertiary); }
.el-card__body { display: flex; gap: 20rpx; }
.el-card__img { width: 160rpx; height: 160rpx; border-radius: var(--jst-radius-sm); background: var(--jst-color-page-bg); }
.el-card__info { flex: 1; min-width: 0; display: flex; flex-direction: column; }
.el-card__name { font-size: 28rpx; font-weight: 700; color: var(--jst-color-text); }
.el-card__qty { margin-top: 6rpx; font-size: 22rpx; color: var(--jst-color-text-tertiary); }
.el-card__price { margin-top: auto; display: flex; align-items: baseline; gap: 12rpx; }
.el-card__points { font-size: 30rpx; font-weight: 800; color: #F5A623; }
.el-card__cash { font-size: 22rpx; color: var(--jst-color-text-tertiary); }
.el-empty { padding: 80rpx; text-align: center; font-size: 24rpx; color: var(--jst-color-text-tertiary); }
