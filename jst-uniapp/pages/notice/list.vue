<template>
  <view class="notice-page">
    <view class="notice-page__header">
      ...
      <view class="notice-page__search" @tap="showSearchComingSoon">🔍</view>
    </view>
    ...
    <view class="notice-page__list">
      ...
      <jst-notice-card v-for="notice in noticeList" :key="notice.noticeId" class="notice-page__card" :notice="notice" @tap="openNotice" />
...
export default {
  ...
  methods: {
    ...
    async fetchNoticeList() {
      const response = await getNoticeList({ category: this.currentCategory || undefined, pageNum: this.pageNum, pageSize: this.pageSize })
    ...
    showSearchComingSoon() { uni.showToast({ title: '搜索功能后续开放', icon: 'none' }) }
  }
}
</script>
