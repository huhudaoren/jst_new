...
> 会话恢复文件。上次更新：2026-04-18（**plan-06 UX 精品化收尾 + 6 张 PATCH 隐患全清** → CORS 通配 + 模板 Picker 自动定位 + CoursePicker + EntityLink 字段对齐 + region 省级字典 + **PATCH-7 销售管理一站式（事务建 sys_user + jst_sales，运营无需双开页面）**；**重启验证 4 项 curl 全过** + **PATCH-7 4 case 全过 + 测试数据已清理**）
...
| **P1 ✅** | PATCH-7 销售管理一站式 | ✅ 完成（2026-04-18，commit `6e6d171`） | 修运营要双开 `/system/user` + `/group-user/sales` 两个页面才能创建销售的体验割裂。新增 `POST /admin/sales/onestop` 端点：同事务里 INSERT sys_user（含 BCrypt password、user_type='00'）+ sys_user_role（按 `asManager` 绑 `jst_sales`/`jst_sales_manager` 角色）+ jst_sales（复用既有 create 走 sales_no 雪花/状态机）。**保留** `jst_sales` 表结构和 SalesScope 切面/提成管线/离职 4 状态机不动；**保留**原 `POST /admin/sales` 端点兼容。前端 `views/jst/sales/index.vue` dialog 加账号字段（用户名 + 初始密码）+ 业务字段（姓名/手机/费率/主管开关）。**跨模块策略**：jst-channel 未依赖 ruoyi-system，用 `JdbcTemplate` 直接操作 sys_user/sys_user_role/sys_role；`PasswordEncoder` 通过 `@Lazy` 注入避开 SecurityConfig→ChannelWebConfig→RestrictedSalesActionInterceptor→SalesService 循环依赖。**顺手补** plan-02 Task 15 遗留 TODO：`SalesResignationServiceImpl.executeResign` 末尾用 JdbcTemplate UPDATE `sys_user.status='1'` 实现"实际离职日账号即时停权"。V2 happy + 重名 + 手机重复 + asManager=true 4 case 全过，三表数据正确，测试数据 4 条已 SQL DELETE 清理。报告 `PATCH-7-SALES-ONESTOP-报告.md`。 |
