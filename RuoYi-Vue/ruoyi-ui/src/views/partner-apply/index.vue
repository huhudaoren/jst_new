<!-- legacy corrupted draft retained inside a comment; active component starts after this block.
<template>
  ...
  <div class="partner-apply-page">
    <section class="hero-section">
      <div class="hero-inner">
        <div class="hero-card">
          <div class="brand-row">
            <img :src="brandLogo" alt="竞赛通" class="brand-logo">
            <span>{{ appTitle }}</span>
          </div>
          <div class="eyebrow">赛事方公开申请入口</div>
          <h1>入驻竞赛通，开通赛事方工作台</h1>
          <p>
            在线提交机构资质、联系人信息与合作材料。平台审核通过后，将进入赛事方独立工作台的账号开通流程。
          </p>
          <div class="hero-actions">
            <el-button type="primary" @click="goApply">立即申请</el-button>
            <el-button plain @click="goStatus">查看申请进度</el-button>
          </div>
          <div v-if="lastApplyNo" class="last-card">
            <span>最近申请单号：{{ lastApplyNo }}</span>
            <el-button type="text" @click="goLastStatus">直接查看</el-button>
          </div>
        </div>

        <div class="lookup-panel">
          <div class="panel-title">进度查询</div>
          <p>当前公开接口按申请单号查询。提交成功后，请妥善保存系统生成的申请编号。</p>
          <el-form ref="lookupForm" :model="lookupForm" :rules="lookupRules" label-position="top">
            <el-form-item label="申请单号" prop="applyNo">
              <el-input
                v-model.trim="lookupForm.applyNo"
    ...
    <section class="content-section">
      <div class="benefit-grid">
        <article v-for="item in benefitList" :key="item.title" class="benefit-card">
          <div class="benefit-icon">{{ item.no }}</div>
          <h3>{{ item.title }}</h3>
          <p>{{ item.desc }}</p>
        </article>
      </div>
      ...
      <div class="flow-card">
        ...
        <p class="flow-note">
          如申请被驳回或要求补件，可根据状态页提示重新整理材料后再次提交。
        </p>
      </div>
    </section>

    <footer class="footer-bar">{{ footerContent }}</footer>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      appTitle: process.env.VUE_APP_TITLE || '竞赛通',
      footerContent: defaultSettings.footerContent,
      brandLogo: require('@/assets/images/light.svg'),
      lastApplyNo: cache.local.get(LAST_APPLY_NO_KEY) || '',
      lookupLoading: false,
      ...
      benefitList: [
        {
          no: '01',
          title: '独立赛事方工作台',
          desc: '审核通过后进入专属后台，管理赛事配置、报名审核、成绩发布与证书相关流程。'
        },
        {
          no: '02',
          title: '数据隔离与权限控制',
          desc: '赛事方数据按主体隔离，后续操作在独立工作台范围内完成，避免跨机构数据混用。'
        },
        {
          no: '03',
          title: '标准化审核闭环',
          desc: '公开申请、平台审核、补件反馈与账号开通形成闭环，降低线下沟通成本。'
        }
      ]
    }
  },
...
.lookup-panel p,
.lookup-panel ::v-deep .el-form-item__label {
  color: rgba(255, 255, 255, 0.82);
}

.lookup-panel ::v-deep .el-input__inner,
.lookup-panel .el-button {
  height: 46px;
  border-radius: 14px;
}
