...
import com.ruoyi.jst.event.service.IJstCertTemplateService;
 ...
 */
@RestController
@RequestMapping("/jst/event/jst_cert_template")
public class JstCertTemplateController extends BaseController
{
     ...
     * 导出证书模板列表
     */
    @PreAuthorize("@ss.hasPermi('jst:event:cert_template:export')")
     ...
     * 获取证书模板详细信息
     */
    @PreAuthorize("@ss.hasPermi('jst:event:cert_template:query')")
     ...
     * 修改证书模板
     */
    @PreAuthorize("@ss.hasPermi('jst:event:cert_template:edit')")
     ...
     * 删除证书模板
     */
    @PreAuthorize("@ss.hasPermi('jst:event:cert_template:remove')")
