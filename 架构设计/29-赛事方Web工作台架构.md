...
## 8. 2026-04-10 PartnerDataScope 补充

### 8.1 新旧两套隔离方式的定位
- `@PartnerScope(field = "partnerId")`：保留给存量接口兼容使用，切面行为仍是“反射覆盖 ReqDTO.partnerId”。
- `@PartnerDataScope`：2026-04-10 起新增赛事方列表/分页查询接口的首选方案，切面会把 SQL 条件写入 `BaseEntity.params.dataScope`，由 Mapper XML 在 `WHERE` 末尾拼接 `${params.dataScope}`。

### 8.2 为什么切到 SQL 层
- 旧方案依赖 DTO 上必须存在 `partnerId` 字段，且 Service / Mapper 仍然需要手工保证不遗漏过滤。
- 新方案把隔离条件直接下沉到 SQL，适合多表关联、列表查询和分页场景，也更贴近 RuoYi 原生 `@DataScope` 的使用方式。

### 8.3 当前约束
- `@PartnerDataScope` 只会处理入参中继承了 `BaseEntity` 的对象；如果查询 DTO 还没继承 `BaseEntity`，需要在对应 PA 业务卡中一并改造。
- 旧 `@PartnerScope` 与新 `@PartnerDataScope` 可以并存；迁移期间不要混用到同一个接口上。
