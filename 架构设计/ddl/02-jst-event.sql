...
CREATE TABLE `jst_contest` (
  ...
  `appointment_capacity`   int(11)       NOT NULL DEFAULT '0'    COMMENT '线下预约场次总名额：0表示不限',
  `writeoff_config`        json          DEFAULT NULL            COMMENT '核销子项配置JSON，支持arrival/gift/ceremony/custom',
  `allow_repeat_appointment` tinyint(1)  NOT NULL DEFAULT '0'    COMMENT '是否允许同一参赛人重复预约：0否 1是',
