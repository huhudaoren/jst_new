...
### 🔴 绝对禁止幻觉
- **禁止编造任何测试结果**。如果你没有实际执行某个 curl/grep/访问，就不许写"✅ 通过"
- **禁止假设接口返回内容**。必须实际 curl 拿到 response 才能判断通过/失败
- **禁止基于文件名推测页面功能**。必须 Read 文件内容确认
- 如果某项测试因为环境原因**无法执行**（服务未启动/超时/网络不通），明确标注 `⏸️ 未执行：原因`，不许标 ✅ 或 ❌
- 如果你不确定某个结果，标注 `❓ 存疑：原因`，不要猜

### 🔴 严谨验证标准
- 每个 ✅ 必须有**证据**（grep 输出 / curl 响应 / 文件内容引用）
- 每个 ❌ 必须有**精确定位**（文件:行号 + 期望值 vs 实际值）
- "文件存在"不等于"功能正常"——必须检查文件内容是否有实质代码，空壳/占位也要标出
- 链路判断必须**逐跳验证**，不许跳步

### 🟢 允许安装工具
- 如果需要 npm 包辅助测试（如 `puppeteer`、`playwright`、`http-client` 等），**允许安装和使用**
- 如果需要 Python 包（如 `requests`、`selenium`），**允许 pip install**
- 但安装的工具**只能用于测试**，不许用来修改业务代码
- 安装操作记录在报告 §附录 中

### 🟢 运行环境信息
- **后端 API 服务**：`http://127.0.0.1:8080`
- **前端用户端**：`http://localhost:8081/#/`（HBuilderX H5 预览，Vue 2 SPA）(google可以预览)
- **前端管理端**：`http://localhost:80` 或 `http://localhost:1024`（ruoyi-ui Vue 2，若已启动）
- **注意**：电脑性能有限，每个页面可能需要加载一会，curl/fetch 请设置**合理超时**（至少 10 秒），不要因为慢就判定失败
- 如果前端服务未启动，Layer 3 标注 `⏸️ 未启动` 即可

### 🟢 新增 Layer 3 — 前端页面真实访问验证
如果前端服务已启动（`http://localhost:8081`），额外做：
1. 用 `curl` 或 `fetch` 访问 `http://localhost:8081/#/` 确认 SPA 可加载
2. 逐页访问关键路由（如 `/#/pages/my/index`），检查是否返回有效 HTML/JS
3. 如果安装了 puppeteer/playwright，可自动化截图验证页面是否白屏
4. 输出每个页面的"可访问性"状态

**但不要过度依赖自动化**——如果工具安装失败或运行异常，回退到手动 curl + grep 即可
...
6. **不许编造测试结果** — 没执行就不许写通过/失败（见 §零 反幻觉准则）
7. **不许因为"大概率没问题"就跳过验证** — 用户肉眼看有很多问题，你必须比用户更仔细
...
5. **报告必须有总览数字 + 健康度评分**
6. **对"空壳文件"和"占位页面"必须单独标注** — .vue 文件存在但内容只有 `<template><view>TODO</view></template>` 的算空壳
7. **实际 curl 的 response 摘要必须附在报告里** — 不能只写"通过"，至少附 HTTP status + body 前 200 字符
8. **超时/慢响应也要记录** — 如果某接口 >15 秒才响应，标注 `⚠️ 慢响应 Xs`
...
### 如果做 Layer 1 + Layer 2 + Layer 3：
1. 先完成 Layer 1 全部
2. 检查后端是否启动（`curl -s -o /dev/null -w "%{http_code}" --connect-timeout 10 http://127.0.0.1:8080`）
3. 如未启动，标注"Layer 2 阻塞：后端未启动"，继续 Layer 3
4. 如已启动，按 §3 逐步执行（curl 超时设 10 秒）
5. 检查前端是否启动（`curl -s -o /dev/null -w "%{http_code}" --connect-timeout 10 http://localhost:8081`）
6. 如已启动，按 §零 Layer 3 执行前端页面访问验证
7. 输出合并报告

### ⚠️ 性能注意事项
- 当前开发机性能有限，**每个页面可能需要较长加载时间**
- curl 默认超时设为 **15 秒**（`--max-time 15`）
- 如果某个请求超时，**重试 1 次**再判定失败
- 不要一次性并发大量请求，**逐条串行**执行
- 如果整体太慢导致测试无法在合理时间完成，优先覆盖**核心链路**（§3.4 的 5 条链路），其余标注"⏸️ 因性能限制未执行"
...
- [ ] 报告中**没有任何一个 ❌ 缺少文件:行号**
- [ ] 所有标注 `⏸️ 未执行` 的项都给出了原因
- [ ] Layer 2 的每个 curl 结果都附了 HTTP status + body 摘要
- [ ] 总览数字与明细条数一致（加总核对）
- [ ] 没有使用"应该""大概""可能"等模糊词来描述测试结果
- [ ] 如果安装了额外工具，已在 §附录 列出
